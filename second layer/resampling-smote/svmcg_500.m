%% svmcg函数
function [bestc, bestAcc, bestMCC, bestSn, bestSp] = svmcg_500(data,label,cstep,cmin,cmax,c0,acc0)
% 生产网格
...
%         [predict_label, accuracy, dec_values] = svmpredict(test_label, test_data,model);
        ...
        for a=1:3000
            if indices(a,1)==i
                test_label0(a,1)=test_label(b,1);
                predict_label0(a,1)=predict_label(b,1);
                b=b+1;
            end
        end
        test_label0=test_label0(1:1743,1);
        predict_label0=predict_label0(1:1743,1);
        [Sn_i,Sp_i,MCC_i,Acc_i]=perf_svm(predict_label0,test_label0);
        Acc(1,i)=Acc_i;
        Sn(1,i)=Sn_i;
        Sp(1,i)=Sp_i;
        MCC(1,i)=MCC_i;
